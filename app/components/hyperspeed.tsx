...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    ...
    myApp.loadAssets().then(() => {
      myApp.init();
      // 🔧 Force a resize once the DOM is fully mounted
      setTimeout(() => {
        myApp.onWindowResize();
      }, 0); // immediate micro-delay to let layout stabilize
    });
  ...
  return (
    <div
      id="lights"
      className="absolute bottom-0 left-0 w-full h-full max-h-screen max-w-screen"
      ref={hyperspeed}
    >
    </div>
  );
